...
@Component({
    selector: 'desktop-file-uploader',
    templateUrl: './desktop-file-uploader.component.html',
    styleUrls: ['./desktop-file-uploader.component.scss'],
    standalone: false
})
export class DesktopFileUploaderComponent implements OnInit {
  ...
  async upload(e) {
    ...
    for (const file of fileListAsArray) {
      ...
      offScreenImage.src = URL.createObjectURL(file[0]);
      ...
      const resizedBlob = await new Promise<NamedBlob>((resolve) =>
        offScreenCanvas.toBlob((blob) => resolve(new NamedBlob(blob, file[0].name)), file[0].type, JPEG_QUALITY)
      );
