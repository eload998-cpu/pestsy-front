...
export class MsdsService extends CrudService {
  ...
  public async downloadFile(id: number): Promise<any> {
    ...
    if ((deviceInfo as unknown as DeviceInfo).platform === "web") {
      ...
      try {
        ...
        this.http.get<Blob>(`${this.baseUrl}/descargar-msds/${id}`, { observe: 'response', responseType: 'blob' as 'json' })
          .pipe(
            catchError(error => {
          ...
          .subscribe(
            () => {
              ...
              if (this.stateCodes.name != 'inactive') {
                ...
                downloadAndroidFile(this.http, `${this.baseUrl}/descargar-msds/${id}`).then((data) => {
                  this.spinner.hide();
