...
export class PermissionService extends CrudService {
  ...
  public async addPermissions(data: Object, arr: Array<string>) {
    let formData = new FormData();
    ...
    return await this.httpClient.post<any>(`${this.baseUrl}/agregar-permisos`, formData).toPromise();
  ...
  public async downloadFile(id: number): Promise<any> {
    ...
    if ((deviceInfo as unknown as DeviceInfo).platform === "web") {
      ...
      try {
        ...
        this.http.get<Blob>(`${this.baseUrl}/descargar-permisos/${id}`, { observe: 'response', responseType: 'blob' as 'json' })
          .pipe(
            catchError(error => {
          ...
          .subscribe(
            () => {
              ...
              if (this.stateCodes.name != 'inactive') {
                ...
                downloadAndroidFile(this.http, `${this.baseUrl}/descargar-permisos/${id}`).then((data) => {
                  this.spinner.hide();
