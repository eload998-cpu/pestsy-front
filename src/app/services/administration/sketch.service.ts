...
export class SketchService extends CrudService {
  ...
  public downloadAllFile(id:number):void
  {
    ...
    this.http.get<Blob>(`${environment.publicUrl}/pdf/descargar-todo-croquis/${id}`, { observe: 'response', responseType: 'blob' as 'json' }).subscribe(
  ...
  public async downloadFile(id: number): Promise<any> {
    ...
    if ((deviceInfo as unknown as DeviceInfo).platform === "web") {
      ...
      try {
        ...
        this.http.get<Blob>(`${this.baseUrl}/descargar-croquis/${id}`, { observe: 'response', responseType: 'blob' as 'json' })
          .pipe(
            catchError(error => {
          ...
          .subscribe(
            () => {
              ...
              if (this.stateCodes.name != 'inactive') {
                ...
                downloadAndroidFile(this.http, `${this.baseUrl}/descargar-croquis/${id}`).then((data) => {
                  this.spinner.hide();
